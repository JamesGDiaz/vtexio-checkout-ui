function addCheckboxesBeforePayButton() {
  ...
  const termsAndConditionsUrl = '/terminos-y-condiciones'
  const privacyPolicyUrl = '/aviso-de-privacidad'
  ...
  <div class="privacy-policy">
    <label for="privacy-policy-accept-checkbox" >
      ...
      <span class="label">Acepto la <a href="${privacyPolicyUrl}" target="_blank">Política de Privacidad</a></span>
    </label >
  </div>
  ...
  const payButtonObserver = () => {
    if (window.privacyPolicyAccepted && window.termsAndConditionsAccepted && !window.incompatiblePaymentOptions) {
  ...
  const setup = function () {
    if ($('.checkout-container').find('.payment-confirmation-wrap').length > 0) {
